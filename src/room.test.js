...
describe("ROOM Class Test", () => {
    const room = new room_1.Room("Suite", 45000, 10);
    ...
    */
    ...
    test("should accept a valid string as a name", () => {
        expect(() => new room_1.Room('Suite', 45000, 10)).not.toThrow();
    });
    test("should reject an empty string", () => {
        expect(() => new room_1.Room('', 45000, 10)).toThrow("Invalid name: please enter an existing room type");
    });
    test("should reject null or undefined values", () => {
        expect(() => new room_1.Room(null, 45000, 10)).toThrow("Invalid name: please enter an existing room type");
        expect(() => new room_1.Room(undefined, 45000, 10)).toThrow("Invalid name: please enter an existing room type");
    });
  ...
  */
    test("Throw error if rate is not a number", () => {
        expect(() => new room_1.Room("Suite", "45000", 10)).toThrow("Rate must be a valid number and greater than 0");
        expect(() => new room_1.Room("Suite", NaN, 10)).toThrow("Rate must be a valid number and greater than 0");
        expect(() => new room_1.Room("Suite", undefined, 10)).toThrow("Rate must be a valid number and greater than 0");
        expect(() => new room_1.Room("Suite", null, 10)).toThrow("Rate must be a valid number and greater than 0");
    });
    test("Throw error if rate is less than or equal to 0", () => {
        expect(() => new room_1.Room("Suite", 0, 10)).toThrow("Rate must be a valid number and greater than 0");
        expect(() => new room_1.Room("Suite", -100, 10)).toThrow("Rate must be a valid number and greater than 0");
    });
  ...
  */
    test("Room rate should be valid, and discounts should be within acceptable range", () => {
        expect(() => new room_1.Room("Suite", 10)).not.toThrow();
        expect(() => new room_1.Room("Suite", 45000, -20)).toThrow("Negative discounts are not possible");
        expect(() => new room_1.Room("Suite", 45000, 120)).toThrow("Discount can't exceed 100%");
    });
    ...
    */
    test("isOccupied should return TRUE if the room is booked on the given date", () => {
        const bookings = new room_1.Booking("Eva Sevillano", "evasevillano@test.com", new Date("2025-03-28"), new Date("2025-03-31"), 10, room);
    ...
    */
    ...
    test("availableRooms should return rooms that are not occupied in the given range", () => {
        const room1 = new room_1.Room("Suite", 45000, 10);
        const room2 = new room_1.Room("Double Bed", 35000, 20);
        const room3 = new room_1.Room("Single", 30000, 5);
        const booking1 = new room_1.Booking("Eva Sevillano", "evasevillano@test.com", new Date("2025-03-06"), new Date("2025-03-08"), 10, room1);
        room1.bookings.push(booking1);
        const booking2 = new room_1.Booking("Kutxi Romero", "kutxiromero@test.com", new Date("2025-03-07"), new Date("2025-03-09"), 15, room2);
        room2.bookings.push(booking2);
        const rooms = [room1, room2, room3];
        const startDate = new Date("2025-03-06");
        const endDate = new Date("2025-03-10");
        const availableRooms = room_1.Room.availableRooms(rooms, startDate, endDate);
    ...
    test("totalOccupancyPercentage should calculate the correct occupancy percentage for all rooms", () => {
        ...
        const booking2 = new room_1.Booking("Kutxi Romero", "kutxiromero@test.com", new Date("2025-03-07"), new Date("2025-03-09"), 15, room2);
        room2.bookings.push(booking2);
        const booking3 = new room_1.Booking("Robe Iniesta", "robeiniesta@test.com", new Date("2025-03-06"), new Date("2025-03-09"), 10, room3);
        room3.bookings.push(booking3);
        const rooms = [room1, room2, room3];
        const startDate = new Date("2025-03-06");
        const endDate = new Date("2025-03-09");
        const totalOccupancy = room_1.Room.totalOccupancyPercentage(rooms, startDate, endDate);
...
describe("BOOKING Class Test", () => {
    const room = new room_1.Room("Suite", 35000, 10);
    // TEST 1: Validate properties format.
    test("Booking properties should be assigned correctly", () => {
        const booking = new room_1.Booking("Eva Sevillano", "evasevillano@test.com", new Date("2025-03-28"), new Date("2025-03-31"), 10, room);
    ...
    */
    test("should throw an error if Name is empty", () => {
        expect(() => new room_1.Booking("", room)).toThrowError("Name cannot be empty");
    });
    it("should throw an error if Name contains numbers", () => {
        expect(() => new room_1.Booking("Eva1234", room)).toThrowError("Name must not contain numbers");
    });
    it("should throw an error if Name contains special characters", () => {
        expect(() => new room_1.Booking("Evit@", room)).toThrowError("Name contains invalid characters");
    });
  ...
  */
    test("Booking with invalid email should throw an error", () => {
        expect(() => {
            new room_1.Booking("Eva Sevillano", "evasevillano", new Date("2025-03-28"), new Date("2025-03-31"), 10, room);
        }).toThrow("Invalid email address!");
    });
    test("Booking with empty email should throw an error", () => {
        expect(() => new room_1.Booking("Eva Sevillano", "", new Date("2025-03-28"), new Date("2025-03-31"), 10, room))
            .toThrow("Invalid email address!");
    });
    test("Booking with a valid email should not throw an error", () => {
        expect(() => {
            new room_1.Booking("Eva Sevillano", "evasevillano@test.com", new Date("2025-03-28"), new Date("2025-03-31"), 10, room);
        }).not.toThrow();
    });
  ...
  */
    test("Booking with the wrong date range should throw an error", () => {
        expect(() => {
            new room_1.Booking("Eva Sevillano", "evasevillano@test.com", new Date("2025-03-31"), new Date("2025-03-28"), 10, room);
    ...
    test("Booking with check-in and check-out on the same date should throw an error", () => {
        expect(() => {
            new room_1.Booking("Eva Sevillano", "evasevillano@test.com", new Date("2025-03-28"), new Date("2025-03-28"), 10, room);
    ...
    */
    ...
    test("Booking fee should calculate correctly without any discount", () => {
        const noDiscountRoom = new room_1.Room("Single", 35000, 0);
        const booking = new room_1.Booking("Eva Sevillano", "evasevillano@test.com", new Date("2025-03-28"), new Date("2025-03-31"), 0, noDiscountRoom);
        expect(booking.fee).toBe(140000);
    });
    test("Booking fee should calculate correctly with 100% discount", () => {
        const fullDiscountRoom = new room_1.Room("Suite", 35000, 100);
        const booking = new room_1.Booking("Eva Sevillano", "evasevillano@test.com", new Date("2025-03-28"), new Date("2025-03-31"), 100, fullDiscountRoom);
